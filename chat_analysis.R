...
pacman::p_load(dplyr, ggplot2, ggforce, rwhatsapp, lubridate, tidyr, stringr,
               ...
               # wordcloud2,
               wordcloud,tm, stringi, RColorBrewer, syuzhet)
...
PHRASE_TO_REMOVE = c(PHRASE_MEDIA_OMITTED,PHRASE_MESSAGE_DELETED,PHRASE_CHANGED_SUBJECT)
...
plot_sentiment_list = lapply(1:2, function(author_num){
      author_name = unique(chat$author)[author_num]
      chat_author = subset(chat, author==author_name)
      ...
      p1 = ggplot(overall_emotion_df, aes(x = "", y = emotion_perc, fill = emotion)) +
        ...
        coord_polar(theta = "y") + theme_void() + ggtitle(author_name)
      ...
      p2 = ggplot(overall_sentiment_df, aes(x = "", y = sentiment_perc, fill = sentiment)) +
        ...
        geom_text(aes(label = sentiment_perc_label),
                  position = position_stack(vjust = 0.5)) +
        scale_fill_manual(values = c(COLOR_PALETTE[2],COLOR_PALETTE[4], "grey"), labels=c("negative","positive","neutral")) + 
        coord_polar(theta = "y") + theme_void() + ggtitle(author_name)
      ...
      # chat_author_sentiment %>% mutate(hour = hour(time)) %>%
      ...
      p4 = ggplot(time_sentiment_df, aes(x = hour, y = value, fill = sentiment)) +
        geom_col() +
        scale_fill_manual(values = c(COLOR_PALETTE[2],COLOR_PALETTE[4], "grey"), labels=c("negative","positive","neutral")) + 
        ggtitle(author_name) + ylab("# messages") + xlab("time [hour]")
...
bovary_v <- get_sentences(chat$text, as_vector = F)
bovary_values <- get_sentiment(bovary_v[1:100])
...
plot_wordcloud = function(chat_text) {
  ...
  corpus_all <- tm_map(corpus_all, removeWords, stopwords(LANGUAGE_SETTING))
